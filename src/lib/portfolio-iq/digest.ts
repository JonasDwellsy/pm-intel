...
export function buildPortfolioIqDigest(input: {
  ...
  const summary = attention
    ...
    : included.length
      ? `${included.length} portfolio update${included.length === 1 ? " is" : "s are"} ready, including ${readiness} setup item${readiness === 1 ? "" : "s"}.`
