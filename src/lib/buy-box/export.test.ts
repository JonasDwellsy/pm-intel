...
const SAMPLE_BUYBOX: ExportBuyBox = {
  id: "test-bb",
...
function buildSampleWorkbook() {
  return buildWorkbook({
    ...
    totalCandidates: 694,
    methodologyVersion: "v0.8",
    liveUrl: "https://pm-intel-chi.vercel.app/buy-boxes/test-bb/results",
...
test("slugifyForFilename handles parentheses + slashes + colons cleanly", () => {
  assert.equal(slugifyForFilename("Test / Buy Box (v2)"), "test-buy-box-v2");
});
...
test("slugifyForFilename falls back to 'buy-box' when input contains no alphanumerics", () => {
  assert.equal(slugifyForFilename("///"), "buy-box");
  assert.equal(slugifyForFilename(""), "buy-box");
});

test("slugifyForFilename normalizes accents into bare ascii", () => {
  // "Resúmé" → "resume". The slugifier strips diacritics before
  // the [^a-z0-9]+ replacement so accented buy-box names produce
...
test("Summary sheet carries buy box name, description, methodology version, and live URL", () => {
  const { workbook } = buildSampleWorkbook();
  ...
  assert.equal(map["Buy box name"], "Scale-Density Rollup");
  ...
  assert.equal(
    ...
    "https://pm-intel-chi.vercel.app/buy-boxes/test-bb/results"
  );
});
...
test("Operators sheet appends 7-Cell (Mixed) suffix when member markets disagree", () => {
  const wb = buildWorkbook({
    ...
    totalCandidates: 100,
    methodologyVersion: "v0.8",
    liveUrl: "https://example.test/buy-boxes/x/results",
...
test("Adaptive columns appear in BOTH data sheets when the buy box references a non-always-on field", () => {
...
test("Summary sheet shows '(none)' for sections with zero criteria", () => {
  const wb = buildWorkbook({
    ...
    totalCandidates: 100,
    methodologyVersion: "v0.8",
    liveUrl: "https://example.test/buy-boxes/empty/results",
    generatedAt: new Date("2026-05-21T00:00:00Z"),
  });
