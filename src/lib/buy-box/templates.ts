// v0.10 — Pre-built buy-box templates. Static JSON in
// src/data/buy-box-templates.json owns the data; this module
...
import templateJson from "@/data/buy-box-templates.json";
import {
  FIELD_REGISTRY,
...
export function getTemplates(): BuyBoxTemplate[] {
 ...
 *  shared references. */
export function getTemplateBySlug(slug: string): BuyBoxTemplate | null {
...
export function summarizeTemplate(template: BuyBoxTemplate): string {
  const parts: string[] = [];
...
export function validateTemplate(template: BuyBoxTemplate): string[] {
  const errors: string[] = [];
  const allCriteria = [
...
function cloneTemplate(t: BuyBoxTemplate): BuyBoxTemplate {
  return {
    slug: t.slug,
    name: t.name,
    tagline: t.tagline,
