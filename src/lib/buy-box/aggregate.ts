...
// one aggregated record so the buy-box results page can show a
// multi-market operator as a single row with summed/averaged
// metrics, then evaluates the buy box against the aggregated row
...
export interface RolledUpEvaluation extends BuyBoxEvaluation {
...
// ─── rollup-aware buy-box evaluation ─────────────────────────────

/** Evaluate a buy box against an AggregatedPMRecord, with ANY-market
...
export function evaluateRollup(
  ...
  if (!agg.isRollup) {
    return evaluateBuyBox(agg, buyBox);
  }
  ...
  const reqSplit = partitionMarket(buyBox.requiredCriteria);
  const excSplit = partitionMarket(buyBox.excludedCriteria);
  const prefSplit = partitionMarket(buyBox.preferredCriteria);
  ...
  // 3. Build a "non-market" buy box for the standard evaluator pass.
  const nonMarketBuyBox: BuyBoxDefinition = {
    ...buyBox,
  ...
  const stdEval = evaluateBuyBox(agg, nonMarketBuyBox);
...
/** Pull marketIds criteria out of a buy box layer; returns both
