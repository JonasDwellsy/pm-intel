 ...
 *  skipped so a stale watch list doesn't crash the export. */
export function resolveAdaptiveColumns(watchList: {
  ...
  const all = [
    ...watchList.requiredCriteria,
    ...watchList.preferredCriteria,
    ...watchList.excludedCriteria,
  ];
  for (const c of all) {
