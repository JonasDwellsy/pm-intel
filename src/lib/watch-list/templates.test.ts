...
test("getTemplateBySlug returns a deep clone — mutating the clone does NOT bleed back", () => {
  const first = getTemplateBySlug("scale-density-rollup") as WatchListTemplate;
  assert.ok(first);
  ...
  const second = getTemplateBySlug("scale-density-rollup") as WatchListTemplate;
...
test("scale-density-rollup matches the v0.10 spec verbatim (field ids, operators, weights)", () => {
  ...
  // in src/data/watch-list-templates.json gets caught before it ships.
  const t = getTemplateBySlug("scale-density-rollup") as WatchListTemplate;
