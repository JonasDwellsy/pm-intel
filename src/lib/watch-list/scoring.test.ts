...
import { evaluateWatchList, type WatchListDefinition } from "./scoring";
...
test("PM that passes all required + has no excluded scores 100 with empty preferred", () => {
  ...
  const result = evaluateWatchList(pm, bb);
...
test("preferred-only watch list — fit score weighted by hit count", () => {
...
test("empty preferred list → fit score 100 (no preferences to differentiate)", () => {
  const pm = makePm();
  const bb = makeWatchList({ requiredCriteria: [{ field: "quadrant7Cell", operator: "eq", value: "SFR Independent" }] });
  const result = evaluateWatchList(pm, bb);
  assert.equal(result.fitScore, 100);
});
