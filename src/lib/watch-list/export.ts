 ...
 *  listed here. */
export interface ExportWatchList {
  id: string;
  name: string;
 ...
 *  separators. Used to slugify a watch list's name into a safe file
...
export function buildFilename(watchListName: string, generatedAt: Date): string {
  return `${slugifyForFilename(watchListName)}-${isoDate(generatedAt)}.xlsx`;
}
...
export function buildWorkbook(args: ExportArgs): ExportResult {
  ...
  const adaptive = resolveAdaptiveColumns({
    requiredCriteria: args.watchList.requiredCriteria,
    preferredCriteria: args.watchList.preferredCriteria,
    excludedCriteria: args.watchList.excludedCriteria,
  });
  ...
  return {
    workbook: wb,
    filename: buildFilename(args.watchList.name, args.generatedAt),
  };
}
...
function buildSummarySheet(args: ExportArgs): WorkSheet {
  ...
  const rows: Array<Array<string | number>> = [
    ["Watch list name", watchList.name],
  ];
  if (watchList.description) {
    rows.push(["Description", watchList.description]);
  }
  rows.push(
  ...
  if (watchList.requiredCriteria.length === 0) {
    ...
    for (const c of watchList.requiredCriteria) {
      rows.push(["", formatCriterion(c)]);
    }
  }
  rows.push([], ["Preferred criteria (weighted)"]);
  if (watchList.preferredCriteria.length === 0) {
    rows.push(["", "(none)"]);
  } else {
    for (const c of watchList.preferredCriteria) {
      rows.push(["", formatCriterion(c)]);
    }
  }
  rows.push([], ["Excluded criteria"]);
  if (watchList.excludedCriteria.length === 0) {
    rows.push(["", "(none)"]);
  } else {
    for (const c of watchList.excludedCriteria) {
      rows.push(["", formatCriterion(c)]);
    }
  }
