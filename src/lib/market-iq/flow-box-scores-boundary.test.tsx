...
describe("observed flow box-score boundary", () => {
  ...
  it("uses the aggregate contract rather than counting capped headline events", () => {
    ...
    assert.doesNotMatch(presentation, /value: activity\.delistings24h/);
    assert.doesNotMatch(presentation, /events\.(?:filter|reduce).*24h/);
  });

  it("withholds unreconciled off-market, standing inventory, and active-rent summaries", () => {
    assert.match(presentation, /Off-market totals, standing active inventory, and active-listing rent summaries remain withheld/);
