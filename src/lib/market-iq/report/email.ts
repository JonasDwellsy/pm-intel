...
export function buildMarketIqReportEmail(input: {
  ...
  const companyProfile = input.report.editorial?.companyProfile;
  const companyCtaUrl = input.report.editorial?.companyCtaUrl;
  const companyCtaLabel = input.report.editorial?.companyCtaLabel || "Talk with our team";
