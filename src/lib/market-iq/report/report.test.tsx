...
import { buildMarketIqReportSnapshot, isPublicMarketIqReportStatus, marketCellKey, parseMarketIqReportSnapshot, type MarketIqMarketObservation } from "./report";
...
function observations(count: number): MarketIqMarketObservation[] {
  return Array.from({ length: count }, (_, index) => ({
    id: `listing-${index}`, propertyKey: `property-${index % 8}`, propertyType: "apartment", bedrooms: 1,
    city: "Cleveland", postalCode: "44114", submarket: "Cleveland", askingRent: 1_000,
    squareFeet: index < 25 ? 700 : null,
  }));
}
...
const baseInput = {
  ...
  scope: { marketId: "cleveland", marketName: "Cleveland-Elyria, OH", submarkets: ["Cleveland"], segments: ["Apartments by bedroom"], periodStart: "2025-08-01", periodEnd: "2026-07-31", totalObservedListings: 40, seededExample: true },
...
describe("Market IQ local market read assembly", () => {
  it("publishes a defensible market cell with level, trajectory, sample, and date", () => {
    const trajectories = new Map([[marketCellKey("Cleveland", "apartment", 1), { rent: 1_025, yearOverYearPct: 2.5, observations: 20, month: "2026-07-01" }]]);
    const report = buildMarketIqReportSnapshot({ ...baseInput, observations: observations(40), trajectories });
    expect(report.marketRead.cells).toHaveLength(1);
    expect(report.marketRead.cells[0]).toMatchObject({ status: "reportable", rentLevel: { medianAskingRent: 1_000, observations: 40, properties: 8, availableThrough: "2026-07-31" }, trajectory: { yearOverYearPct: 2.5, observations: 20 } });
  ...
  it("suppresses a thin rent-level cell and withholds a thin trajectory", () => {
    const trajectories = new Map([[marketCellKey("Cleveland", "apartment", 1), { rent: 1_025, yearOverYearPct: 2.5, observations: 9, month: "2026-07-01" }]]);
    const report = buildMarketIqReportSnapshot({ ...baseInput, observations: observations(29), trajectories });
    expect(report.marketRead.cells[0]).toMatchObject({ status: "suppressed", trajectory: null, rentLevel: { medianAskingRent: null } });
    expect(report.marketRead.cells[0].suppressionReason).toMatch(/30 observed listings/);
  });
...
describe("Market IQ report email", () => {
  it("keeps the message PM-branded and market-only", () => {
    ...
    expect(email.html).toContain("View market read");
