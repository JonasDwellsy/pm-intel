// GET /api/brief-digest/unsubscribe?u=<userId>&t=<hmac> — one-click unsubscribe
// from the market-brief digest. Stateless: verifies the HMAC token, upserts
// BriefDigestPreference.unsubscribed = true (independent of the watch-list
// digest's opt-out). Renders a plain confirmation page.
...
export async function GET(req: Request) {
  ...
  if (!userId || !verifyBriefUnsubToken(userId, token)) {
    return page("Link invalid", "This unsubscribe link is invalid or has expired. Please use the link from your most recent brief email.");
  }
