...
function dateTimeLabel(value: Date): string {
  return value.toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit", timeZone: "UTC", timeZoneName: "short" });
}
...
export default async function PilotSuccessPage() {
  ...
  return <div className="mx-auto max-w-[1100px] px-6 pb-16">
    ...
    <div className="mt-8 space-y-5">{pilots.map((pilot) => <article key={pilot.id} className="overflow-hidden rounded-xl border border-grid bg-white shadow-sm">
      ...
      <div className="grid border-t border-grid lg:grid-cols-[360px_1fr]">
        ...
        <section className="p-5 sm:p-6">
          ...
          <div className="mt-4 grid grid-cols-2 overflow-hidden rounded-lg border border-grid sm:grid-cols-4">{[
            ["Delivered", pilot.deliveredEmailCount], ["Clicked", pilot.clickedEmailCount], ["Directional opens", pilot.directionalOpenCount], ["Failed", pilot.failedEmailCount],
          ].map(([label, value]) => <div key={String(label)} className="border-b border-r border-grid p-3 last:border-r-0 sm:border-b-0"><p className="text-[9px] font-bold uppercase tracking-wider text-grey-500">{label}</p><p className="mt-1 text-lg font-semibold text-navy">{value}</p></div>)}</div>
          ...
          <div className="mt-5"><p className="text-[10px] font-bold uppercase tracking-wider text-grey-500">Account activity</p><div className="mt-3 space-y-3">{pilot.activity.map((item) => item.activityType === "intervention" ? <div key={`work-${item.id}`} className="rounded-lg border border-grid p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full border px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${statusClass(item.status, item.priority)}`}>{item.priority === "overdue" ? "Overdue" : item.status}</span><span className="text-[10px] font-bold uppercase tracking-wider text-grey-500">{item.kind.replace("_", " ")}</span></div><h4 className="mt-2 text-sm font-semibold text-navy">{item.title}</h4>{item.note && <p className="mt-1 text-xs leading-5 text-grey-600">{item.note}</p>}<p className="mt-2 text-[11px] text-grey-500">{dateTimeLabel(item.occurredAt)} · {item.assignedTo ? `Assigned to ${item.assignedTo}` : "Unassigned"}{item.dueAt ? ` · Due ${dateLabel(item.dueAt)}` : ""}</p></div>{item.status !== "completed" && <form action={updatePilotInterventionStatus}><input type="hidden" name="portfolioId" value={pilot.id} /><input type="hidden" name="interventionId" value={item.id} /><input type="hidden" name="status" value="completed" /><button className="rounded-md border border-grid bg-white px-3 py-2 text-[11px] font-semibold text-navy">Mark complete</button></form>}</div></div> : <div key={`email-${item.id}`} className={`rounded-lg border p-4 ${["bounce", "dropped", "spamreport", "unsubscribe"].includes(item.eventType) ? "border-rose-200 bg-rose-50" : "border-grid bg-surface-soft"}`}><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-grid bg-white px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-navy">SendGrid</span><span className="text-[10px] font-bold uppercase tracking-wider text-grey-500">{item.messageKind === "owner_digest" ? "Owner briefing" : "PM brief"}</span></div><h4 className="mt-2 text-sm font-semibold capitalize text-navy">{item.eventType}</h4>{item.reason && <p className="mt-1 text-xs leading-5 text-grey-600">{item.reason}</p>}<p className="mt-2 text-[11px] text-grey-500">{dateTimeLabel(item.occurredAt)} · {item.engagementStrength === "explicit" ? "Observed click event" : item.engagementStrength === "directional" ? "Directional signal only" : "Delivery operations"}</p></div>)}{pilot.activity.length === 0 && <p className="rounded-lg border border-dashed border-grid px-4 py-8 text-center text-xs text-grey-500">No staff or communication activity has been recorded yet.</p>}</div></div>
        </section>
      </div>
    ...
    <section className="mt-8 rounded-xl border border-grid bg-surface-soft p-5"><p className="text-[10px] font-bold uppercase tracking-wider text-grey-500">Measurement boundary</p><p className="mt-2 text-xs leading-5 text-grey-600">Workspace views are recorded directly. Email delivery and failures require signed SendGrid webhook evidence. Opens are retained as directional signals only. Clicks are observed interaction events and can include automated security scanning, so neither is counted as proof of product value. Value remains grounded in useful feedback, PM responses, and reviewed outcomes.</p></section>
  </div>;
}
