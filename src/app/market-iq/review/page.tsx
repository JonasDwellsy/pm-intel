...
export default async function MarketIqReviewInboxPage({
  ...
  searchParams: Promise<{ check?: string; draft?: string; draftId?: string; market?: string }>;
}) {
  ...
  const [drafts, recentDecisions, orchestrationItems] = await Promise.all([
    ...
    prisma.marketIqEditionDraft.findMany({
      where: { organizationId, marketId: activeMarket.id, status: { in: ["published", "dismissed"] } },
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    <nav className="mb-6 flex items-center gap-2 text-xs font-semibold text-slate-500"><Link href="/market-iq" className="hover:text-teal-700">Market IQ</Link><span>/</span><Link href={`/market-iq/editions?market=${encodeURIComponent(activeMarket.id)}`} className="hover:text-teal-700">Client reports</Link><span>/</span><span>Review</span></nav>
    <MarketIqMarketSelector markets={entitledMarkets} activeMarketId={activeMarket.id} basePath="/market-iq/review" />
    ...
    <header className="grid gap-7 border-b border-grid pb-9 lg:grid-cols-[1fr_380px] lg:items-end">
      <div><p className="dq-eyebrow">Report review · {activeMarket.shortLabel}</p><h1 className="dq-h1">Review the next client edition</h1><p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">New drafts appear here when the monthly rent data advances. Review the evidence, edit the wording, or skip an edition.</p></div>
