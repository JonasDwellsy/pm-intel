...
export default async function MarketIqReportComposerPage({ searchParams }: { searchParams: Promise<{ published?: string; delivery?: string }> }) {
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    ...
    <header className="grid gap-7 border-b border-grid pb-8 lg:grid-cols-[1fr_360px] lg:items-end">
      <div><p className="dq-eyebrow">Client advisory</p><h1 className="dq-h1">Prepare a Cleveland local market read</h1><p className="mt-3 max-w-3xl text-[15px] leading-6 text-muted-foreground">Turn observed local rent levels and validated Trends trajectory into a client-ready read under your firm’s brand. Publishing freezes the evidence and creates a revocable public link.</p></div>
    ...
    <section className="mt-8 grid gap-7 xl:grid-cols-[370px_1fr]">
      <aside className="space-y-6">
        <form action={publishMarketIqReport} className="rounded-xl border border-grid bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 grid gap-4">
            ...
            <label className="text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">Local scope<select name="portfolioScope" defaultValue="cleveland-local-read" className="mt-2 w-full rounded-md border border-grid bg-surface-soft px-3 py-2.5 text-sm font-normal normal-case tracking-normal text-navy"><option value="cleveland-local-read">Cleveland, Lakewood, and Euclid</option></select></label>
            <label className="text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">Segments<select name="segments" defaultValue="houses-apartments" className="mt-2 w-full rounded-md border border-grid bg-surface-soft px-3 py-2.5 text-sm font-normal normal-case tracking-normal text-navy"><option value="houses-apartments">Houses and apartments by bedroom</option></select></label>
      ...
      <div style={style} className="overflow-hidden rounded-2xl border border-grid bg-[#f7f6f2] shadow-sm">
        ...
        <div className="p-6 sm:p-9 lg:p-12">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--composer-accent)]">{snapshot.scope.marketName}</p><h2 className="mt-4 max-w-3xl text-4xl font-semibold leading-tight tracking-tight text-[var(--composer-primary)]">Your local asking market, observed clearly</h2><p className="mt-4 max-w-3xl text-sm leading-6 text-slate-600">{snapshot.marketRead.narrative}</p>
          <div className="mt-8 grid gap-4 md:grid-cols-3">{snapshot.marketRead.cells.filter((cell) => cell.status === "reportable").slice(0, 6).map((cell) => <article key={cell.key} className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-slate-500">{cell.geographyLabel} · {cell.label}</p><p className="mt-4 text-2xl font-semibold text-[var(--composer-primary)]">{money(cell.rentLevel.medianAskingRent)}</p><p className="mt-1 text-xs font-semibold text-[var(--composer-accent)]">{cell.trajectory ? `${cell.trajectory.yearOverYearPct >= 0 ? "+" : ""}${cell.trajectory.yearOverYearPct.toFixed(1)}% year over year` : "Trajectory not published"}</p><p className="mt-4 border-t border-slate-100 pt-3 text-xs text-slate-500">{cell.rentLevel.observations.toLocaleString()} observed listings · through {cell.rentLevel.availableThrough}</p></article>)}</div>
