...
export default async function MarketIqInternalAdminPage() {
  ...
  const rows = organizations.map((org) => {
    ...
    return {
      ...
      recovery: recoverySignal({ active, setupAt: org.marketIqWorkspacePreference?.onboardingCompletedAt ?? null, reportAt, testStatus: test?.status ?? null, testAt: test?.sentAt ?? null, recipientAt, campaignStatus: campaign?.status ?? null, campaignAt: campaign?.createdAt ?? null, sendStatus: send?.deliveryStatus ?? null, lastActivity }),
    };
  });
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    ...
    {recoveryRows.length > 0 && <section className="mt-8 overflow-hidden rounded-2xl border border-orange-200 bg-orange-50"><div className="flex flex-col gap-3 border-b border-orange-200 px-6 py-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="dq-eyebrow text-orange-800">Workspace triage</p><h2 className="dq-h2">Customers that need attention</h2></div><p className="text-xs text-slate-600">Ordered by delivery failures, onboarding stalls, then inactivity.</p></div><div className="divide-y divide-orange-200">{recoveryRows.map((row) => <article key={row.id} className="grid gap-4 px-6 py-5 lg:grid-cols-[110px_1fr_1.2fr_170px] lg:items-center"><span className={`w-fit rounded-full px-3 py-1.5 text-[9px] font-bold uppercase tracking-wider ${row.recovery.level === "Critical" ? "bg-red-100 text-red-800" : row.recovery.level === "High" ? "bg-orange-200 text-orange-900" : "bg-white text-slate-600"}`}>{row.recovery.level}</span><div><p className="font-semibold text-navy">{row.brandProfile?.displayName ?? row.name}</p><p className="mt-1 text-xs text-slate-500">{row.progress} of 7 steps · last activity {shortDate(row.lastActivity)}</p></div><div><p className="text-sm font-semibold text-orange-950">{row.recovery.reason}</p><p className="mt-1 text-xs text-slate-600">{row.recovery.action}</p></div><Link href={`/market-iq/internal/admin/${row.id}`} className="rounded-md border border-orange-300 bg-white px-4 py-2 text-center text-xs font-semibold text-navy">Open support view</Link></article>)}</div></section>}
