...
export default async function MarketIqInternalAdminPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    <nav className="flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-500"><Link href="/market-iq/internal/readiness">Operations readiness</Link><span>/</span><Link href="/market-iq/internal/data-operations">Data loading</Link><span>/</span><span>Market IQ admin</span></nav>
    ...
    <section className="mt-8" aria-labelledby="operations-control-center"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="dq-eyebrow">Internal tools</p><h2 id="operations-control-center" className="dq-h2">Operations control center</h2></div><p className="max-w-xl text-sm leading-6 text-slate-500">Monitor launch dependencies, nightly data collection, and pilot adoption without leaving the admin workspace.</p></div><div className="mt-5 grid gap-4 lg:grid-cols-3"><AdminTool href="/market-iq/internal/readiness" eyebrow="Platform health" title="Operations readiness" description="Verify deployment identity, configuration, source evidence, delivery services, billing, and launch workflow dependencies." action="Review readiness" /><AdminTool href="/market-iq/internal/data-operations" eyebrow="Nightly ingestion" title="Data loading" description="Track seven-day completeness for the 25-market cohort, detailed listing feeds, missing dates, and recent failures." action="Review data health" /><AdminTool href="/market-iq/internal/pilot-telemetry" eyebrow="Pilot activity" title="Pilot telemetry" description="Follow customer journey milestones and operational failures while preserving the product's privacy boundary." action="Review telemetry" /></div></section>
...
function AdminTool({ href, eyebrow, title, description, action }: { href: string; eyebrow: string; title: string; description: string; action: string }) {
  return <Link href={href} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-teal-300 hover:shadow-md"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-teal-700">{eyebrow}</p><h3 className="mt-3 text-xl font-semibold text-navy">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{description}</p><p className="mt-5 text-sm font-semibold text-teal-700">{action} <span aria-hidden className="transition-transform group-hover:translate-x-1">→</span></p></Link>;
}
