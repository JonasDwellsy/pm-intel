...
export default async function MarketIqWorkspaceSupportPage({ params }: { params: Promise<{ organizationId: string }> }) {
  ...
  const [usage, markets] = await Promise.all([
    prisma.usageEvent.findMany({ where: { orgId: organization.clerkOrgId, eventName: "market_iq_page_view", occurredAt: { gte: since } }, orderBy: { occurredAt: "desc" }, select: { userId: true, targetSlug: true, occurredAt: true } }),
    prisma.market.findMany({ select: { id: true, city: true, state: true }, orderBy: [{ state: "asc" }, { city: "asc" }] }),
  ]);
  const identityIds = [...new Set(organization.memberships.map((membership) => membership.userId))];
  ...
  if (identityIds.length) {
    ...
    } catch { /* Identity enrichment is best effort. */ }
  }
