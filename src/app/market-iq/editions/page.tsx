...
export default async function MarketIqEditionsPage({ searchParams }: { searchParams: Promise<{ activated?: string; refresh?: string; enrollment?: string }> }) {
  ...
  const [composer, recipients, recentReports, recurringDraft, latestOrchestration, organizationSetup, organizationHasAccess] = await Promise.all([
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    ...
    <section className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      {recentReports.length ? <div className="divide-y divide-slate-100">{recentReports.map((report) => {
        ...
        return <article key={report.id} className="grid gap-5 px-6 py-6 sm:px-8 lg:grid-cols-[1fr_190px_210px] lg:items-center"><div><div className="flex flex-wrap items-center gap-3"><p className="text-lg font-semibold text-navy">Cleveland market read</p><span className={`rounded-full px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider ${report.status === "published" ? "bg-emerald-50 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>{report.status}</span></div><p className="mt-2 text-sm text-slate-600">{report.periodLabel} · {audience}</p><p className="mt-2 text-xs text-slate-400">Published {dateLabel(report.publishedAt)} · {report.sends.length} confirmed {report.sends.length === 1 ? "send" : "sends"}</p></div><div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Distribution</p><p className="mt-2 text-sm font-semibold capitalize text-navy">{campaign?.status ?? "Not staged"}</p><p className="mt-1 text-xs text-slate-500">{campaign?._count.recipients ?? 0} selected</p></div><div className="flex flex-wrap gap-2 lg:justify-end">{report.status === "published" && <><Link href={`/reports/market/${report.publicToken}`} target="_blank" className="rounded-md border border-slate-300 px-3 py-2 text-xs font-semibold text-navy">Open client page</Link>{campaign ? <Link href={`/market-iq/published/${campaign.id}`} className="rounded-md bg-navy px-3 py-2 text-xs font-semibold text-white">Open report receipt</Link> : <Link href="/market-iq/distribution" className="rounded-md bg-navy px-3 py-2 text-xs font-semibold text-white">Prepare delivery</Link>}</>}</div></article>;
      })}</div> : <div className="px-6 py-10 text-center sm:px-8"><p className="text-lg font-semibold text-navy">No client reports yet</p><p className="mt-2 text-sm text-slate-500">Create the first report from the current Cleveland market read.</p></div>}
    </section>
