...
export default async function MarketIqSharingPage() {
  ...
  const [recipients, reports, sends, campaigns] = await Promise.all([
    ...
    prisma.marketIqReport.findMany({ where: { organizationId, status: "published" }, orderBy: { publishedAt: "desc" }, take: 24, select: { id: true, periodLabel: true, publishedAt: true, publicToken: true } }),
    prisma.marketIqReportSend.findMany({ where: { organizationId }, orderBy: { createdAt: "desc" }, take: 12, select: { id: true, deliveryStatus: true, sentAt: true, deliveredAt: true, lastEmailEventType: true, recipient: { select: { name: true, email: true } }, report: { select: { periodLabel: true } } } }),
    prisma.marketIqDistributionCampaign.findMany({ where: { organizationId }, orderBy: { createdAt: "desc" }, take: 12, select: { id: true, status: true, createdAt: true, report: { select: { periodLabel: true } }, _count: { select: { recipients: true } } } }),
  ]);
  const latest = reports[0];

  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500"><Link href="/market-iq" className="hover:text-teal-700">Market IQ</Link><span>/</span><span>Sharing</span></nav>
    <header className="mt-6 grid gap-6 border-b border-grid pb-8 lg:grid-cols-[1fr_360px] lg:items-end"><div><p className="dq-eyebrow">Sharing</p><h1 className="dq-h1">Prepare and track client delivery</h1><p className="mt-3 max-w-3xl text-[15px] leading-6 text-slate-600">Choose a published market read, confirm its audience, and follow delivery status. Your recipient directory is managed separately.</p></div><aside className="rounded-xl border border-teal-200 bg-teal-50 p-5"><p className="text-xs font-bold uppercase tracking-wider text-teal-800">Recipient directory</p><p className="mt-2 text-sm leading-6 text-slate-700">{recipients} saved {recipients === 1 ? "recipient" : "recipients"}. Add people or import a spreadsheet before preparing a delivery.</p><Link href="/market-iq/distribution" className="mt-3 inline-flex text-sm font-semibold text-teal-800">Manage recipients →</Link></aside></header>

    <section className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><p className="dq-eyebrow">Latest published read</p>{latest ? <><h2 className="dq-h2">{latest.periodLabel}</h2><p className="mt-2 text-sm text-slate-500">Published {latest.publishedAt?.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric", timeZone: "UTC" })}</p><div className="mt-5 flex flex-wrap gap-3"><Link href={`/reports/market/${latest.publicToken}`} target="_blank" className="rounded-md border border-slate-300 px-4 py-2.5 text-sm font-semibold text-navy">Review client view</Link><form action={startMarketIqDistributionCampaign}><input type="hidden" name="reportId" value={latest.id} /><button className="rounded-md bg-navy px-4 py-2.5 text-sm font-semibold text-white">Prepare delivery</button></form></div></> : <><h2 className="dq-h2">No published read yet</h2><p className="mt-2 text-sm text-slate-500">Publish a client report before preparing delivery.</p><Link href="/market-iq/editions" className="mt-5 inline-flex rounded-md bg-navy px-4 py-2.5 text-sm font-semibold text-white">Open client reports</Link></>}</article>
