...
export default async function MarketIqPerformancePage() {
  ...
  const [organization, reports, sends, recipients, campaigns] = await Promise.all([
  ...
  const delivered = sends.filter((send) => Boolean(send.deliveredAt));
  const failed = sends.filter((send) => send.deliveryStatus === "failed" || (send.lastEmailEventType ? FAILURE_EVENTS.has(send.lastEmailEventType) : false));
  const observedEngagement = sends.filter((send) => send.lastEmailEventType ? ENGAGEMENT_EVENTS.has(send.lastEmailEventType) : false);
  ...
  const reportRows = reports.map((report) => {
    ...
    return {
      ...
      delivered: reportSends.filter((send) => Boolean(send.deliveredAt)).length,
      failed: reportSends.filter((send) => send.deliveryStatus === "failed" || (send.lastEmailEventType ? FAILURE_EVENTS.has(send.lastEmailEventType) : false)).length,
      engaged: reportSends.filter((send) => send.lastEmailEventType ? ENGAGEMENT_EVENTS.has(send.lastEmailEventType) : false).length,
    };
  });
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-7 lg:px-10 lg:py-12">
    ...
    <section className="mt-8 grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
      ...
      <article className="rounded-2xl border border-slate-200 bg-navy p-6 text-white shadow-sm sm:p-8"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-orange-300">Observed engagement</p><p className="mt-3 text-5xl font-semibold">{observedEngagement.length}</p><p className="mt-3 text-sm leading-6 text-white/70">deliveries currently have an open or click as their latest SendGrid event.</p><div className="mt-8 border-t border-white/15 pt-5"><p className="text-sm font-semibold">Directional, not a complete engagement rate</p><p className="mt-2 text-xs leading-5 text-white/55">Market IQ stores each delivery’s latest provider event. A later event can replace an earlier open, so this count is useful for follow-up but not campaign attribution.</p></div></article>
    </section>

    <section className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 p-6 sm:p-8"><div><p className="dq-eyebrow">Edition performance</p><h2 className="dq-h2">One delivery record for each market read</h2></div><Link href="/market-iq/editions" className="text-sm font-semibold text-teal-700">Open client reports →</Link></div>{reportRows.length ? <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-slate-50 text-[10px] uppercase tracking-[0.12em] text-slate-500"><tr><th className="px-6 py-4">Edition</th><th className="px-4 py-4">Published</th><th className="px-4 py-4">Audience</th><th className="px-4 py-4">Delivered</th><th className="px-4 py-4">Issues</th><th className="px-4 py-4">Latest engagement</th><th className="px-6 py-4">Campaign</th></tr></thead><tbody className="divide-y divide-slate-100">{reportRows.map((report) => <tr key={report.id}><td className="px-6 py-4"><Link href={`/reports/market/${report.publicToken}`} className="font-semibold text-navy hover:text-teal-700">{report.periodLabel}</Link></td><td className="px-4 py-4 text-slate-500">{shortDate(report.publishedAt)}</td><td className="px-4 py-4 font-semibold text-navy">{report.audience}</td><td className="px-4 py-4"><span className="font-semibold text-teal-700">{report.delivered}</span>{report.audience > 0 && <span className="ml-1 text-xs text-slate-400">({percent(report.delivered, report.audience)})</span>}</td><td className="px-4 py-4 font-semibold text-amber-700">{report.failed}</td><td className="px-4 py-4 text-slate-600">{report.engaged}</td><td className="px-6 py-4"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-600">{report.campaign?.status ?? "Not staged"}</span></td></tr>)}</tbody></table></div> : <p className="p-8 text-sm text-slate-500">No published editions fall inside this reporting window.</p>}</section>
    ...
    <p className="mt-8 rounded-xl border border-slate-200 bg-slate-50 px-5 py-4 text-xs leading-5 text-slate-500"><strong className="text-slate-700">Evidence note:</strong> Delivery status, bounces, suppression, and the latest open or click come from SendGrid events attached to Market IQ report sends. This page does not use tracking pixels on public reports and does not infer whether a recipient read or acted on the market analysis.</p>
  </main>;
}
