...
export default async function MarketIqRecipientsPage({ searchParams }: { searchParams: Promise<{ saved?: string; flow?: string; imported?: string; updated?: string }> }) {
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    ...
    <section className="mt-8 grid gap-6 xl:grid-cols-[340px_1fr]">
      <form id="add-recipient" action={saveMarketIqRecipient} className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">{query.flow === "launch" && <input type="hidden" name="returnTo" value="launch" />}<p className="dq-eyebrow">Add one person</p><h2 className="dq-h2">Save a recipient</h2><div className="mt-5 grid gap-4"><label className="text-sm font-semibold text-navy">Name<input name="name" required maxLength={120} autoComplete="name" className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2.5 font-normal" /></label><label className="text-sm font-semibold text-navy">Company, optional<input name="companyName" maxLength={160} autoComplete="organization" className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2.5 font-normal" /></label><label className="text-sm font-semibold text-navy">Email<input name="email" required type="email" inputMode="email" autoComplete="email" maxLength={254} pattern="[^\\s@]+@[^\\s@]+\\.[^\\s@]+" title="Enter a complete email address such as name@company.com" className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2.5 font-normal" /></label><label className="text-sm font-semibold text-navy">Relationship<select name="kind" defaultValue="client" className="mt-2 w-full rounded-md border border-slate-300 bg-white px-3 py-2.5 font-normal"><option value="client">Current client</option><option value="prospect">Prospect</option></select></label><label className="flex items-center gap-3 rounded-xl border border-teal-200 bg-teal-50 p-3 text-xs font-semibold text-navy"><input type="checkbox" name="approveRecurringDelivery" value="1" />Add to monthly delivery</label><button className="rounded-md bg-navy px-4 py-3 text-sm font-semibold text-white">Save recipient</button><p className="text-xs leading-5 text-slate-500">Addresses are normalized before saving. Using the same email updates the existing record.</p></div></form>
