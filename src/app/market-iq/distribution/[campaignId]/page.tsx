...
export default async function MarketIqCampaignPage({
  ...
  searchParams: Promise<{ stage?: string; delivery?: string; flow?: string }>;
}) {
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    {query.flow === "launch" && <MarketIqLaunchJourney current="delivery" />}
    <nav className="mt-5 flex items-center gap-2 text-xs font-semibold text-slate-500"><Link href="/market-iq">Market IQ</Link><span>/</span><Link href="/market-iq/sharing">Sharing</Link><span>/</span><span>{campaign.report.periodLabel}</span></nav>
    <header className="mt-6 grid gap-6 border-b border-grid pb-8 lg:grid-cols-[1fr_360px] lg:items-end"><div><p className="dq-eyebrow">Recipients and email</p><h1 className="dq-h1">Review each recipient before delivery</h1><p className="mt-3 max-w-3xl text-[15px] leading-6 text-slate-600">Adjust the audience, review the PM-branded email, and confirm each recipient. Nothing is sent until you approve it.</p></div><aside className="rounded-xl bg-navy p-5 text-white"><p className="text-[10px] font-bold uppercase tracking-wider text-white/55">Edition</p><p className="mt-2 text-lg font-semibold">{campaign.report.periodLabel}</p><p className="mt-2 text-sm text-white/65">{campaign.recipients.length} selected · {campaign.status}</p></aside></header>
    {query.delivery && <p className={`mt-6 rounded-xl border px-5 py-3 text-sm font-semibold ${query.delivery === "sent" || query.delivery === "already_sent" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : query.delivery === "failed" || query.delivery === "suppressed" ? "border-rose-200 bg-rose-50 text-rose-800" : "border-amber-200 bg-amber-50 text-amber-900"}`}>{query.delivery === "sent" ? "The email was accepted for delivery." : query.delivery === "already_sent" ? "No duplicate was sent. This edition was already delivered to that recipient." : query.delivery === "failed" ? "Delivery failed. Review the error before retrying." : query.delivery === "suppressed" ? "Delivery was blocked because the recipient is suppressed." : "No email was sent because the recipient status had already changed."}</p>}
    ...
    <section className="mt-8 grid gap-7 xl:grid-cols-[420px_1fr]">
      <form action={saveMarketIqCampaignAudience} className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {query.flow === "launch" && <input type="hidden" name="flow" value="launch" />}
