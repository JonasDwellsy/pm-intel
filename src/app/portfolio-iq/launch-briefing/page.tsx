...
import { approvePortfolioLaunchBriefing } from "../actions";
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
      ...
      <section className={`mt-10 rounded-xl border p-6 ${briefing.isApproved ? "border-emerald-200 bg-emerald-50" : "border-navy bg-navy text-white"}`}>
        {briefing.isApproved ? <div className="flex flex-wrap items-center justify-between gap-5"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-emerald-800">Monitoring baseline active</p><h2 className="mt-2 text-2xl font-semibold text-navy">Your portfolio is launched</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-emerald-900/80">Today and future briefings can now measure new evidence against this approved starting position.</p></div><Link href="/today" className="rounded-md bg-navy px-5 py-3 text-sm font-semibold text-white">Open Today →</Link></div> : <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-teal-200">Launch approval</p><h2 className="mt-2 text-2xl font-semibold">Establish this as the monitoring baseline</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-white/75">Approval freezes the evidence shown above. Clearly labeled activation gaps remain in Dwellsy&apos;s queue and do not become unsupported conclusions.</p></div><form action={approvePortfolioLaunchBriefing}><input type="hidden" name="portfolioId" value={snapshot.portfolio.id} /><button className="rounded-md bg-white px-5 py-3 text-sm font-semibold text-navy">Approve and launch portfolio</button></form></div>}
      </section>
    </main>
  );
}
