...
export default async function OwnerReportsPage({ searchParams }: { searchParams: Promise<{ email?: string }> }) {
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    ...
    {data.briefingChanges.length > 0 && <section className="mt-10 rounded-xl border border-teal/30 bg-teal-soft p-5 sm:p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="dq-eyebrow">Delivery delta</p><h2 className="dq-h2">New since your last briefing</h2><p className="mt-2 text-sm leading-6 text-foreground/70">This uses the last successful email delivery as its cutoff. Reading activity in the app does not suppress this briefing section.</p></div><Link href="/portfolio-iq/watchlists/activity" className="text-sm font-semibold text-teal-800 print:hidden">Open activity ledger →</Link></div><div className="mt-5 grid gap-3 lg:grid-cols-2">{data.briefingChanges.map((event) => <Link key={event.id} href={event.href} className="rounded-lg border border-grid bg-white p-4 print:block"><div className="flex gap-2 text-[10px] font-bold uppercase tracking-wider text-teal-700"><span>{event.kind}</span><span className="text-muted-foreground">{event.severity}</span></div><h3 className="mt-2 font-semibold text-navy">{event.headline}</h3><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{event.detail}</p></Link>)}</div></section>}
