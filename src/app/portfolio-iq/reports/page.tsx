...
function deliveredFindingCount(value: string | null): number | null {
  if (!value) return null;
  try {
    const snapshot = JSON.parse(value) as { attention?: unknown[] };
    return Array.isArray(snapshot.attention) ? snapshot.attention.length : null;
  } catch {
    return null;
  }
}
...
export default async function OwnerReportsPage({ searchParams }: { searchParams: Promise<{ email?: string }> }) {
  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    ...
    {data.briefingChanges.length > 0 && <section className="mt-10 rounded-xl border border-teal/30 bg-teal-soft p-5 sm:p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="dq-eyebrow">Delivery delta</p><h2 className="dq-h2">New since your last briefing</h2><p className="mt-2 text-sm leading-6 text-foreground/70">Related evidence, decision activity, and outcomes appear as one finding. This uses the last successful email delivery as its cutoff.</p></div><Link href="/portfolio-iq/watchlists/activity" className="text-sm font-semibold text-teal-800 print:hidden">Open activity ledger →</Link></div><div className="mt-5 grid gap-3 lg:grid-cols-2">{data.briefingChanges.map((event) => <Link key={event.id} href={event.href} className="rounded-lg border border-grid bg-white p-4 print:block"><div className="flex gap-2 text-[10px] font-bold uppercase tracking-wider text-teal-700"><span>{event.eventCount > 1 ? `Connected finding · ${event.eventCount} updates` : event.kind}</span><span className="text-muted-foreground">{event.severity}</span></div><h3 className="mt-2 font-semibold text-navy">{event.headline}</h3><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{event.detail}</p></Link>)}</div></section>}
    ...
    <section className="mt-10 grid gap-6 border-t border-grid pt-8 lg:grid-cols-[1fr_0.8fr]"><article><p className="dq-eyebrow">Evidence health</p><h2 className="dq-h2">What this briefing can support</h2><div className="mt-4 grid gap-3 sm:grid-cols-3">{snapshot.sources.map((source) => <div key={source.label} className="rounded-lg border border-grid bg-white p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-navy">{source.label}</p><span className={`h-2.5 w-2.5 rounded-full ${source.status === "current" ? "bg-teal-600" : "bg-amber-500"}`} /></div><p className="mt-2 text-xs leading-5 text-muted-foreground">{source.detail}</p></div>)}</div><p className="mt-4 text-xs leading-5 text-muted-foreground">This briefing uses advertised asking-market evidence. It does not measure occupancy, signed leases, concessions, effective rent, or NOI.</p></article><aside className="rounded-xl border border-grid bg-surface-soft p-5 sm:p-6"><p className="dq-eyebrow">Delivery control</p><h2 className="dq-h2">Weekly owner email</h2><form action={updatePortfolioDigestPreference} className="mt-4 rounded-lg border border-grid bg-white p-4"><input type="hidden" name="portfolioId" value={snapshot.portfolio.id} /><label className="flex items-start gap-3 text-sm font-semibold text-navy"><input type="checkbox" name="enabled" defaultChecked={data.digestPreference?.enabled ?? false} className="mt-1" /><span>Email the signed-in user when the briefing materially changes.<span className="mt-1 block text-xs font-normal leading-5 text-muted-foreground">Weekly cadence. Off by default. Identical briefings are suppressed.</span></span></label><button className="mt-4 rounded-md bg-navy px-3 py-2 text-xs font-semibold text-white">Save delivery setting</button></form>{data.deliveries.length ? <div className="mt-4 divide-y divide-grid">{data.deliveries.map((delivery) => { const findingCount = deliveredFindingCount(delivery.snapshot); return <div key={delivery.id} className="flex items-start justify-between gap-4 py-3 text-sm"><div><p className="font-semibold capitalize text-navy">{delivery.triggerKind} · {delivery.status}</p><p className="text-xs text-muted-foreground">{dateLabel(delivery.createdAt)}{findingCount === null ? "" : ` · ${findingCount} findings`} · v{delivery.briefingVersion}</p></div><p className="max-w-[190px] truncate text-xs text-muted-foreground">{delivery.email}</p></div>; })}</div> : <p className="mt-4 text-sm leading-6 text-muted-foreground">No owner briefing has been delivered yet.</p>}<p className="mt-4 text-xs leading-5 text-muted-foreground">{data.digestPreference?.enabled ? "Weekly delivery is enabled. A new email is sent only after the briefing evidence or decision state changes." : "Weekly delivery is off. Preview sends remain available only to your signed-in account email."}</p></aside></section>
  </main>;
}
