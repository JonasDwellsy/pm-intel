...
export default async function PortfolioChangesPage() {
  ...
  const sourceHealth = monitoring.latestRun?.sourceHealth ?? "not run";
  const sourceHealthLabel = sourceHealth === "healthy" ? "Healthy" : sourceHealth === "unchanged" ? "No new source period" : sourceHealth === "unavailable" ? "Unavailable" : "Awaiting first run";
  return (
    <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
      <DwellsyIqWorkspaceNav />
      ...
      <section aria-label="Change summary" className="mt-7 overflow-hidden rounded-xl border border-grid bg-white shadow-sm">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4">
          {[["High priority", String(monitoring.comparison.highPriorityCount), "Prompt owner review"], ["Properties affected", String(monitoring.comparison.affectedAssetCount), `of ${monitoring.portfolio.assetCount} assets`], ["Source health", sourceHealthLabel, monitoring.latestRun ? `Last checked ${dateLabel(monitoring.latestRun.startedAt)}` : "No automated run yet"], ["Weekly snapshot", monitoring.currentPeriodCaptured ? "Saved" : "Ready", monitoring.currentPeriodKey]].map(([label, value, detail]) => <article key={label} className="border-b border-grid px-5 py-5 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0"><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-muted-foreground">{label}</p><p className="mt-2 text-[22px] font-semibold tracking-tight text-navy">{value}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{detail}</p></article>)}
        </div>
      </section>

      {sourceHealth !== "healthy" && sourceHealth !== "not run" && <section className={`mt-6 rounded-xl border px-5 py-4 ${sourceHealth === "unavailable" ? "border-rose-200 bg-rose-50" : "border-amber-200 bg-amber-50"}`}><p className="text-sm font-semibold text-navy">{sourceHealth === "unavailable" ? "Current source evidence is unavailable" : "No newer source period is available"}</p><p className="mt-1 text-xs leading-5 text-foreground/75">Performance alerts were neither created nor resolved from this run. Operational changes such as a confirmed property identity or operator assignment can still be recorded without pretending the asking market moved.</p></section>}
