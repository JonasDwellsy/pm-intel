...
export default async function TodayPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
      ...
      {routedActivity.routed.length > 0 && <section className="mt-7 rounded-xl border border-teal/30 bg-teal-soft p-5 sm:p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="dq-eyebrow">Since your last review</p><h2 className="dq-h2">New watched findings</h2><p className="mt-2 text-sm leading-6 text-foreground/70">Related evidence, decisions, and outcomes are combined into one issue. The detailed event history remains in the activity ledger.</p></div><Link href="/portfolio-iq/watchlists/activity" className="rounded-md bg-navy px-4 py-2.5 text-sm font-semibold text-white">Review all {routedActivity.eligibleUnreadCount}</Link></div><div className="mt-5 grid gap-3 lg:grid-cols-2">{routedActivity.routed.map((event) => <Link key={event.id} href={event.href} className="rounded-lg border border-grid bg-white p-4 transition-colors hover:border-teal/40"><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-teal-700"><span>{event.eventCount > 1 ? "Connected finding" : event.kind}</span>{event.eventCount > 1 && <span className="rounded-full bg-teal-soft px-2 py-0.5 text-teal-800">{event.eventCount} updates</span>}<span className="text-muted-foreground">{event.severity}</span></div><h3 className="mt-2 font-semibold leading-6 text-navy">{event.headline}</h3><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{event.detail}</p></Link>)}</div></section>}
