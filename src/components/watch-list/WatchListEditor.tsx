...
// Watch List editor — owns the entire form state (name, description,
...
//   - Live preview: debounced 500ms POST /api/watch-lists/preview
//     that drives the sticky bottom match-count strip + the
//     top-10 side panel.
//   - Save → POST /api/watch-lists (create) or PUT /api/watch-lists/[id]
//     (update). On create the parent router pushes us into edit
//     mode; on update we stay on page and flash a toast.
//   - Delete (edit mode) → confirm modal → DELETE → redirect to /watch-lists.
//
// The component is self-contained: pages mount it with either an
// `initialWatchList` (edit) or `null` (new) and pass through the market
...
import { FIELD_REGISTRY } from "@/lib/watch-list/fields";
import { isCriterionComplete } from "@/lib/watch-list/validation";
import type { MarketOption } from "@/lib/watch-list/editor-options";
import {
  clearPendingDraft,
  consumePendingDraft,
  savePendingDraft,
} from "@/lib/watch-list/pending-draft";
import { CriterionRow, type Layer } from "./CriterionRow";

export interface EditorWatchList {
  id: string;
  name: string;
...
interface Props {
  /** null for /watch-lists/new, populated for /watch-lists/[id]/edit. */
  initial: EditorWatchList | null;
...
export function WatchListEditor({
  ...
  // Auth state. Editing existing watch lists is already protected by
  // middleware (server-side); the editor itself runs in /watch-lists/new
  ...
  // state under the key dwellsy:pendingWatchListDraft; on the
  // round-trip back to /watch-lists/new?template=<slug> we apply
  ...
  React.useEffect(() => {
    const handle = window.setTimeout(async () => {
      // Don't bother previewing an empty watch list.
      ...
      try {
        const res = await fetch("/api/watch-lists/preview", {
          method: "POST",
  ...
  async function handleSave(options: { andApply?: boolean } = {}) {
    ...
    // /watch-lists/new stays public, but Save requires a real account.
    ...
    try {
      ...
      if (isEdit && initial) {
        const res = await fetch(`/api/watch-lists/${initial.id}`, {
          method: "PUT",
          headers: { "content-type": "application/json" },
        ...
        flashSavedState();
        showToast("success", "Watch list created.");
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/watch-lists/${initial.id}`, { method: "DELETE" });
    ...
    <div className="bg-background pb-32">
        ...
        <div className="mt-8 rounded-lg border border-grid bg-white p-6">
          ...
          <div className="mt-5">
            ...
            <textarea
              ...
              rows={2}
              placeholder="Optional — describe what this watch list targets."
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 border-t border-grid bg-white/95 backdrop-blur">
        {/* Anon-save explainer. Appears above the action row only on
            /watch-lists/new for signed-out users so they know the
        ...
        {!isEdit && isSignedIn !== true && (
          <div className="border-b border-grid bg-surface-soft">
            <p className="mx-auto max-w-[1080px] px-6 py-2 text-[12px] text-foreground/70">
              You&rsquo;ll need an account to save watch lists. Your edits
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {saving ? (
                ...
                // labelled "Create watch list" and getting bounced.
