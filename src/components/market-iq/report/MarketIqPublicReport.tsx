...
function SummaryCard({ cell }: { cell: MarketIqMarketCell }) {
  ...
  return <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)] sm:p-8">
    ...
    <p className="mt-4 border-t border-slate-100 pt-4 text-xs leading-5 text-slate-500">Trends IQ · N={cell.observations.toLocaleString()} · {month(cell.month)} · temporary median basis</p>
  </article>;
}
...
export function MarketIqPublicReport({ report, publicToken, preview = false }: {
  ...
  return <div role={preview ? undefined : "main"} style={reportStyle} className="min-h-screen bg-[#f7f6f2] text-slate-900">
    ...
    <div className="mx-auto max-w-7xl px-6 py-12 lg:px-10 lg:py-16">
      ...
      <section className="mt-16"><div className="grid gap-4 lg:grid-cols-[0.75fr_1.25fr] lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--report-accent)]">Local market map</p><h2 className="mt-3 text-3xl font-semibold tracking-tight text-[var(--report-primary)]">See where the market diverges</h2></div><p className="max-w-2xl leading-7 text-slate-600 lg:justify-self-end">Start with all apartments or all houses, then move into bedroom detail. Each marker is based on its own ZIP-level Trends IQ sample.</p></div><div className="mt-8"><MarketIqRentMap points={report.marketMap.points} primaryColor={report.brand.primaryColor} accentColor={report.brand.accentColor} /></div></section>
      ...
      <section className="mt-16 rounded-3xl border border-amber-200 bg-amber-50 p-7"><p className="text-xs font-bold uppercase tracking-[0.18em] text-amber-800">Temporary summary basis</p><h2 className="mt-3 text-xl font-semibold text-[var(--report-primary)]">Overall product summaries use the 999-row median for now</h2><p className="mt-3 max-w-4xl leading-7 text-slate-700">The MSA, municipality, and ZIP summaries labeled “All apartments” or “All houses” use the median stored on the Trends IQ 999-bedroom rows. Year-over-year change is an exact comparison with the matching prior-year median in that same Trends series. This temporary adapter will be replaced with the canonical Trends IQ fields when the data science backfill is complete. Bedroom-specific prices continue to use the canonical Trends IQ values.</p></section>
