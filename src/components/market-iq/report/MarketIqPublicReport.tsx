...
export function MarketIqPublicReport({ report, publicToken, preview = false }: {
  ...
  return <div role={preview ? undefined : "main"} style={reportStyle} className="min-h-screen bg-[#f7f6f2] text-slate-900">
    <header className={`${preview ? "relative" : "sticky top-0"} z-30 border-b border-slate-200/90 bg-white/95 backdrop-blur`}>
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-6 py-4 lg:px-10">
        <div>{report.brand.logoUrl ? <>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={report.brand.logoUrl} alt={report.brand.displayName} className="max-h-10 max-w-[210px] object-contain object-left" />
        </> : <p className="text-lg font-bold tracking-tight text-[var(--report-primary)]">{report.brand.displayName}</p>}<p className="mt-0.5 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Interactive local market read</p></div>
        <div className="flex items-center gap-4 text-right text-xs text-slate-500"><div className="hidden sm:block"><p>Rent data through {trendsSource?.availableThrough ?? report.scope.periodEnd}</p><p className="mt-1">Prepared {new Date(report.generatedAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</p></div>{publicToken && !preview ? <a href={`/reports/market/${publicToken}/pdf`} className="rounded-md border border-slate-300 bg-white px-3 py-2 font-semibold text-slate-600 hover:border-slate-500">PDF export</a> : <span className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 font-semibold text-slate-400">Preview</span>}</div>
      </div>
    </header>
