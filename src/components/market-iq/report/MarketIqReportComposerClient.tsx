    ...
    <section className="mt-8 grid gap-7 xl:grid-cols-[390px_1fr]">
      <form action={publishMarketIqReport} className="h-fit rounded-xl border border-grid bg-white p-5 shadow-sm">
        ...
        <div className={activeStep === 1 ? "" : "hidden"}>
          <p className="dq-eyebrow">Step 1 of 4</p><h2 className="dq-h2">Choose the evidence</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Start with the saved Cleveland scope, then remove anything that does not belong in this client conversation.</p>
          <div className="mt-5 grid gap-5">
            ...
            <fieldset><div className="flex items-center justify-between gap-3"><legend className="text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">Cities</legend><button type="button" onClick={() => setSelection((current) => ({ ...current, cities: current.cities.length === MARKET_IQ_REPORT_CITIES.length ? [] : [...MARKET_IQ_REPORT_CITIES] }))} className="text-xs font-semibold text-teal-700">{selection.cities.length === MARKET_IQ_REPORT_CITIES.length ? "Clear" : "Select all"}</button></div><div className="mt-2 grid gap-2">{MARKET_IQ_REPORT_CITIES.map((city) => <ScopeOption key={city} name="cities" value={city} label={city} checked={selection.cities.includes(city)} onChange={() => setSelection((current) => ({ ...current, cities: toggleValue(current.cities, city) }))} />)}</div></fieldset>
            <fieldset><div className="flex items-center justify-between gap-3"><legend className="text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">ZIP codes</legend><button type="button" onClick={() => setSelection((current) => ({ ...current, zipCodes: current.zipCodes.length === MARKET_IQ_REPORT_ZIPS.length ? [] : [...MARKET_IQ_REPORT_ZIPS] }))} className="text-xs font-semibold text-teal-700">{selection.zipCodes.length === MARKET_IQ_REPORT_ZIPS.length ? "Clear" : "Select all"}</button></div><p className="mt-1 text-xs text-muted-foreground">{selection.zipCodes.length} of {MARKET_IQ_REPORT_ZIPS.length} selected</p><div className="mt-2 grid max-h-64 grid-cols-2 gap-2 overflow-y-auto rounded-lg border border-grid p-2">{MARKET_IQ_REPORT_ZIPS.map((zip) => <ScopeOption key={zip} name="zipCodes" value={zip} label={zip} checked={selection.zipCodes.includes(zip)} onChange={() => setSelection((current) => ({ ...current, zipCodes: toggleValue(current.zipCodes, zip) }))} />)}</div></fieldset>
