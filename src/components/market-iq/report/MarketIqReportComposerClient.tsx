...
export function MarketIqReportComposerClient({ snapshot, initialBrand, initialEditorialDefaults, initialSelection, source, priorEdition, draftId = null, launchFlow = false }: {
  ...
  const steps = [
    ...
    { id: 2 as const, label: "Review and publish", detail: `${coverage.counts.reportable} rent values` },
  ];
  ...
  return <>
    <section aria-label="Client edition progress" className="mt-8 overflow-hidden rounded-2xl border border-grid bg-white shadow-sm">
      <div className="grid gap-px bg-grid sm:grid-cols-2">{steps.map((step) => <button key={step.id} type="button" onClick={() => setActiveStep(step.id)} className={`bg-white p-5 text-left transition ${activeStep === step.id ? "shadow-[inset_0_-3px_0_#1b6e8c]" : "hover:bg-slate-50"}`}><span className={`grid h-7 w-7 place-items-center rounded-full text-xs font-bold ${activeStep === step.id ? "bg-navy text-white" : activeStep > step.id ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-500"}`}>{activeStep > step.id ? "✓" : step.id}</span><p className="mt-3 text-sm font-semibold text-navy">{step.label}</p><p className="mt-1 truncate text-xs text-muted-foreground">{step.detail}</p></button>)}</div>
    </section>
    <section className="mt-8 grid gap-7 xl:grid-cols-[390px_1fr]">
      <form action={publishMarketIqReport} className="h-fit rounded-xl border border-grid bg-white p-5 shadow-sm">
        ...
        <div className={activeStep === 1 ? "" : "hidden"}>
          <p className="dq-eyebrow">Step 1 of 2</p><h2 className="dq-h2">Add your perspective</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">The market read works without commentary. Add a concise point of view when it helps your client understand why the data matters.</p>
          <div className="mt-5 grid gap-5">
            ...
            <div className="rounded-xl border border-grid bg-white p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground">Firm identity</p><p className="mt-2 text-sm font-semibold text-navy">{brand.displayName}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Your saved logo, colors, contact details, and company profile will be applied.</p></div><a href={`/market-iq/get-started?market=${encodeURIComponent(snapshot.scope.marketId)}&step=1`} className="shrink-0 text-xs font-semibold text-teal-700">Edit firm profile</a></div></div>
            <div className="flex gap-3"><a href={`/market-iq/get-started?market=${encodeURIComponent(snapshot.scope.marketId)}&step=2`} className="flex-1 rounded-md border border-grid px-4 py-3 text-center text-sm font-semibold text-navy">Change scope</a><button type="button" onClick={() => setActiveStep(2)} className="flex-1 rounded-md bg-navy px-4 py-3 text-sm font-semibold text-white">Review edition</button></div>
          </div>
        </div>

        <div className={activeStep === 2 ? "" : "hidden"}>
          <p className="dq-eyebrow">Step 2 of 2</p><h2 className="dq-h2">Review and publish</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Publication freezes the market read, wording, and branding into a permanent, revocable client link.</p>
          ...
          <button type="button" onClick={() => setActiveStep(1)} className="mt-3 w-full px-4 py-2 text-sm font-semibold text-teal-700">Back to your perspective</button>
        </div>
      </form>
