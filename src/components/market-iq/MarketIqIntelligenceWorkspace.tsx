...
export function MarketIqIntelligenceWorkspace({ report, market, listingSync, listingSupplyHistory = [], clientAdvisoryEnabled }: { report: MarketIqReportSnapshot; market: MarketIqMarketDefinition; listingSync: ListingSync; listingSupplyHistory?: ListingSupplyHistoryPoint[]; clientAdvisoryEnabled: boolean }) {
  ...
  const listingAgeBuckets = listingSync.listingAgeBuckets ?? [];
  const listingUnavailableLabel = listingSync.unavailableReason === "missing"
    ? "Snapshot not captured"
    : listingSync.unavailableReason === "stale"
      ? "Snapshot is stale"
      : listingSync.unavailableReason === "invalid"
        ? "Snapshot is incomplete"
        : listingSync.unavailableReason === "read_failed"
          ? "Snapshot read failed"
          : "Listing snapshot unavailable";
  const unavailableCuts = report.marketRead.unavailableCuts ?? [];
  const maxAgeBucket = Math.max(1, ...listingAgeBuckets.map((bucket) => bucket.count));
  ...
  const hasSupplyTrend = listingSupplyHistory.length >= 2;
    ...
    <section id="inventory" className="mt-14 scroll-mt-28">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5">{hasSupplyTrend ? <SupplyTrendLine points={listingSupplyHistory} metric="inventory" label="Daily active inventory history" /> : <div className="rounded-xl border border-slate-200 bg-slate-50 p-5"><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-slate-400">Current inventory mix</p><div className="mt-5 flex h-4 overflow-hidden rounded-full bg-slate-200"><div className="bg-[#1b6e8c]" style={{ width: `${apartmentShare}%` }} /><div className="bg-[#c16f36]" style={{ width: `${inventoryTotal > 0 ? 100 - apartmentShare : 0}%` }} /></div><div className="mt-4 grid grid-cols-2 gap-4"><div><p className="text-2xl font-semibold text-navy">{listingSync.apartmentListings.toLocaleString()}</p><p className="mt-1 text-sm text-slate-500">Apartments</p></div><div><p className="text-2xl font-semibold text-navy">{listingSync.houseListings.toLocaleString()}</p><p className="mt-1 text-sm text-slate-500">Houses</p></div></div></div>}</div>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5">{hasSupplyTrend ? <SupplyTrendLine points={listingSupplyHistory} metric="age" label="Daily median observed listing-age history" /> : <div className="rounded-xl border border-slate-200 bg-slate-50 p-5"><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-slate-400">Current active listing age</p>{listingAgeBuckets.length > 0 ? <div className="mt-4 space-y-3">{listingAgeBuckets.map((bucket) => <div key={bucket.key} className="grid grid-cols-[68px_minmax(0,1fr)_48px] items-center gap-3"><p className="text-xs font-semibold text-slate-500">{bucket.label}</p><div className="h-2.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-teal" style={{ width: `${(bucket.count / maxAgeBucket) * 100}%` }} /></div><p className="text-right text-xs font-semibold tabular-nums text-navy">{bucket.count.toLocaleString()}</p></div>)}</div> : <p className="mt-4 text-sm text-slate-500">Listing lifecycle timestamps are not available in this observation.</p>}</div>}</div>
        </article>
      </div>
      ...
      {hasSupplyTrend && <div className="mt-4 grid gap-4 lg:grid-cols-2">
