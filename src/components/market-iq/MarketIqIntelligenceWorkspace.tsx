  ...
  return <main style={{ "--report-primary": "#17324a", "--report-accent": "#c16f36" } as CSSProperties} className="mx-auto w-full max-w-[1500px] px-5 py-8 sm:px-6 lg:px-10 lg:py-10">
    <header className="overflow-hidden rounded-3xl bg-navy text-white shadow-[0_24px_70px_rgba(15,31,63,0.18)]">
      <div className="grid gap-8 px-6 py-8 sm:px-9 sm:py-10 lg:grid-cols-[1fr_340px] lg:items-end lg:px-12">
        <div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-white/55">Internal market intelligence</p><h1 className="mt-4 max-w-4xl text-4xl font-semibold tracking-tight sm:text-5xl">{market.name} rental market</h1><p className="mt-4 max-w-3xl text-base leading-7 text-white/72">Explore asking-rent direction across the MSA, municipalities, ZIPs, and product segments. Use this workspace for analysis, then decide what belongs in a client report.</p>
        ...
        <aside className="rounded-2xl border border-white/15 bg-white/10 p-5 backdrop-blur-sm"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/55">Latest Trends month</p><p className="mt-2 text-3xl font-semibold">{month(latestMonth)}</p><p className="mt-3 text-sm leading-6 text-white/65">Rent levels and changes on this page come directly from Dwellsy Trends.</p>{clientAdvisoryEnabled ? <Link href={`/market-iq/report?from=market-read&market=${encodeURIComponent(market.id)}`} className="mt-5 block rounded-lg bg-white px-4 py-3 text-center text-sm font-bold text-navy hover:bg-white/90">Create a client edition</Link> : <Link href="/market-iq/subscribe?upgrade=client_advisory" className="mt-5 block rounded-lg bg-white px-4 py-3 text-center text-sm font-bold text-navy hover:bg-white/90">Add client reporting</Link>}</aside>
      </div>
    </header>
