...
export function MarketIqActivationFlow({ marketId, marketLabel, snapshot, initialBrand, initialEditorialDefaults, initialSelection, initialStep, source, completed, clientAdvisoryEnabled, logoStorageEnabled }: { marketId: string; marketLabel: string; snapshot: MarketIqReportSnapshot; initialBrand: Brand; initialEditorialDefaults: MarketIqEditorialDefaults; initialSelection: MarketIqReportScopeSelection; initialStep: number; source: "dwellsy_trends" | "verified_seed" | "unavailable"; completed: boolean; clientAdvisoryEnabled: boolean; logoStorageEnabled: boolean }) {
  ...
  return <form ref={formRef} action={completeMarketIqActivation} className="mt-8">
    ...
    <input type="hidden" name="sourceAvailable" value={source === "dwellsy_trends" ? "1" : "0"} />
    ...
    {step === 1 && <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2"><p className="text-sm font-semibold text-navy">Company logo, optional</p><div className="mt-2 flex flex-wrap items-center gap-4 rounded-xl border border-slate-200 p-4">{brand.logoUrl ? <img src={brand.logoUrl} alt="Current company logo" className="h-14 max-w-48 object-contain" /> : <div className="flex h-14 w-28 items-center justify-center rounded-lg bg-slate-50 text-xs font-semibold text-slate-400">No logo yet</div>}<label className={`rounded-md border border-slate-300 px-4 py-2.5 text-sm font-semibold ${logoStorageEnabled ? "cursor-pointer text-navy" : "cursor-not-allowed bg-slate-50 text-slate-400"}`}>Upload logo<input type="file" accept="image/png,image/jpeg,image/webp" disabled={!logoStorageEnabled} className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadLogo(file); }} /></label>{brand.logoUrl && <button type="button" onClick={() => updateBrand("logoUrl", null)} className="text-sm font-semibold text-slate-500">Remove</button>}</div><p className={`mt-1 text-xs ${logoStorageEnabled ? "text-slate-400" : "font-semibold text-amber-700"}`}>{logoStorageEnabled ? "PNG, JPEG, or WebP, up to 2 MB." : "Logo storage is not connected in this environment. You can finish setup now and add a logo later."}</p></div>
