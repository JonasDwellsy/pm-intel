...
# v0.24 — facts-not-judgments voice (restores the neutralization from the
# reverted PR #117 exec-summary work; only that part was reverted, the voice
# fix itself was wanted). States each dimension's position RELATIVE TO THE
# COHORT MEDIAN as a fact ("above the cohort median on lease-up speed and rent
# growth; below on …") instead of verdict language ("ranks top quartile,
# driven by top-quartile operational discipline"). HARD CONSTRAINT: never
# surface rank or the precise composite anywhere on the scorecard — so this
# sentence states neither quartile nor rank. The reader judges.
_DIM_LABELS = {
    "dom": "lease-up speed",
